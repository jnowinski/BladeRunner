...
Supports DeepSeek-V3 with very low cost pricing.
"""
import os
import sys
import time
...
from Scraper.config import DEEPSEEK_CONFIG

try:
    from openai import OpenAI  # DeepSeek uses OpenAI-compatible API
    from tenacity import retry, stop_after_attempt, wait_exponential
except ImportError as e:
    raise ImportError(f"Missing required packages. Run: pip install openai tenacity\n{e}")
...
class DeepSeekClient:
    """
    DeepSeek client for synthetic text generation.
    Very cost-effective Chinese LLM with strong capabilities.
    """
    
    # Pricing per 1M tokens (DeepSeek-V3)
    PRICING = {
    ...
    def __init__(self):
        ...
        if not DEEPSEEK_CONFIG['api_key']:
            raise ValueError("DeepSeek API key not configured. Check .env file.")
    ...
    def generate_post(
        ...
        try:
            ...
            logger.info(f"Generated post with DeepSeek: {output_tokens} tokens, ${cost:.4f}, {elapsed_time:.2f}s")
            ...
            logger.error(f"Error generating post with DeepSeek: {e}")
            raise
